...
2. 启动客户端，首先在「服务器选择」页填写服务器地址，点击「连接」进行健康检查（自动补全 `http://` 前缀）
3. 连接成功后进入登录页；如有本地会话则自动恢复，否则输入用户名和密码登录（需先在 Web 界面注册账号）

登录页显示当前服务器，可点「更换」返回服务器选择页；登录后也可在「设置 → 服务器」中修改地址，保存后会断开连接并回到服务器选择页。
